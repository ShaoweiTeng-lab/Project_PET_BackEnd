...
public class ActivityDaoImpl {
    ...
    public List<ActivityRes> getHotActivities() {
        ...
        List<ActivityRes> activityList =
                template.query(sql, params, (rs, rowNum) -> {
                    ...
                    activityRes.setTitle(rs.getString("ACTIVITY_TITILE"));
    ...
    //退出活動
    public String leaveActivity(int userId, JoinReq joinReq) {

        String querySql = "select * FROM activity\n" +
                "where activity_id = :activityId";

        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("activityId", joinReq.getActivityId());
        ...
        if (querydate.getDay() - nowdate.getDay() < 3) {
            ...
            userMap.put("activityId", joinReq.getActivityId());
            ...
            countMap.put("activityId", joinReq.getActivityId());
